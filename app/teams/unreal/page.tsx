...
import { teams, projects } from "@/lib/data";

// Получаем данные о команде из lib/data.ts
const team = teams.find(t => t.id === "unreal");
// Фильтруем проекты только для этой команды
const teamProjects = projects.filter(p => p.team === "unreal");

// Преобразуем данные для членов команды
const teamMembers = team?.members.map((member, index) => ({
  id: index + 1,
  name: member.name,
  role: member.position,
  bio: "Специалист в разработке на Unreal Engine и создании игровых проектов.",
  avatarUrl: member.avatar,
  avatarFallback: member.name.split(' ').map(part => part[0]).join('')
}));

export default function UnrealTeamPage() {
  ...
  return (
    <div className="flex flex-col min-h-screen">
      ...
      <main className="flex flex-col w-full">
        {/* Hero section */}
        <section className={`w-full py-24 relative bg-gradient-to-br from-blue-600 to-indigo-600`}>
                  ...
                  <Card className="h-full flex flex-col">
                    <CardContent className="p-6 flex flex-col items-center text-center flex-grow">
                      <Avatar className="h-24 w-24 mb-4">
                        ...
                        <AvatarFallback className="text-2xl bg-blue-500/10">{member.avatarFallback}</AvatarFallback>
                      </Avatar>
                      <h3 className="text-xl font-semibold mb-1">{member.name}</h3>
                      <p className="text-sm text-muted-foreground mb-4">{member.role}</p>
                      <p className="text-sm">Specialist in Unreal Engine development and game project creation.</p>
                    </CardContent>
        ...
        <section className="py-20 bg-background">
          <div className="container mx-auto px-4">
            <div className="text-center mb-12">
              ...
              <p className="text-lg text-muted-foreground max-w-2xl mx-auto">
                Examples of our work in game development and interactive applications
              </p>
            </div>
        ...
        {/* CTA Section */}
        <section className="py-20 bg-blue-500/10">
          <div className="container mx-auto px-4">
            <div className="max-w-3xl mx-auto text-center">
              <h2 className="text-3xl font-bold mb-6">Ready to Start Your Project?</h2>
              <p className="text-lg text-muted-foreground mb-8">
                Contact our Unreal Engine team today to discuss how we can help bring your vision to life
              </p>
